...
double CalcDerivative(Eigen::VectorXd coeffs, double x_input)
{	 
	 return coeffs[1]+(coeffs[2]*2*x_input)+(coeffs[3]*3*x_input*x_input);
}

AD<double> CalcDerivativeAD(Eigen::VectorXd coeffs, AD<double> x_input)
{	 
	 return coeffs[1]+(coeffs[2]*2*x_input)+(coeffs[3]*3*x_input*x_input);
}
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	...
	// the cost function should be implemented here!
	...
	for (unsigned int t = 1; t < N; t++) {
	  ...
	  AD<double> psides0 = CppAD::atan(CalcDerivativeAD(coeffs,x0));
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  vars[cte_start] = polyeval(coeffs, state[0]) - state[1];
  vars[epsi_start] = state[2] - atan(CalcDerivative(coeffs,state[0]));
  
  /*std::cout << "vars " << vars << std::endl;
  std::cout << "coeffs " << coeffs << std::endl;
  std::cout << "tang = " << CalcDerivative(coeffs,state[0]) << std::endl;
  std::cout << "poly_y = " << polyeval(coeffs, state[0]) << std::endl;*/
